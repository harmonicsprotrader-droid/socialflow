...
async function loadDashboard() {
  try {
    ...
    if (history.length === 0) {
      el.innerHTML = '<div class="empty-state">No posts yet.<br>Go to Discover Feeds to get started!</div>';
    } else {
    ...
    el.innerHTML = feeds.map(f => `
      <div class="feed-item">
        ...
        <div class="feed-actions">
          <button class="btn btn-ghost" onclick="checkFeedNow(${f.id}, event)" title="Check now">↻</button>
...
async function toggleFeedActive(id, active) {
  ...
  const feed = feeds.find(f => f.id === id);
  if (!feed) return;
  await fetch(`/api/feeds/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...feed, active }) });
  loadFeeds();
}
